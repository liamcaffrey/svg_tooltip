...
# df.facet_data_unpivot$tooltip <- sprintf("Tour Date: %s<br>Rate/'000: %s<br>Misses: %s"
										# ,df.facet_data_unpivot$calendar_day
										# ,round(df.facet_data_unpivot$rate_per_thousand, 0)
										# ,df.facet_data_unpivot$misses
										# )
df.facet_data_unpivot$tooltip <- sprintf("Tour Date: %s<br>Rate/'000: %s<br>Misses: %s<br>Hits: %s<br>Total/Key: %s<br>Total/Day: %s"
										,df.facet_data_unpivot$calendar_day
										,round(df.facet_data_unpivot$rate_per_thousand, 0)
										,df.facet_data_unpivot$misses
										,df.facet_data_unpivot$hits
										,df.facet_data_unpivot$total_key_count
										,df.facet_data_unpivot$total_date_count
										)
# df.facet_data_unpivot$tooltip <- sprintf("Id: %s<br>Tour Date: %s<br>Rate/'000: %s<br>Misses: %s<br>Hits: %s<br>Total/Key: %s<br>Total/Day: %s"
										# ,as.character(df.facet_data_unpivot$Id)
										# ,as.character(df.facet_data_unpivot$calendar_day)
										# ,as.character(round(df.facet_data_unpivot$rate_per_thousand, 0))
										# ,as.character(df.facet_data_unpivot$misses)
										# ,as.character(df.facet_data_unpivot$hits)
										# ,as.character(df.facet_data_unpivot$total_key_count)
										# ,as.character(df.facet_data_unpivot$total_date_count)
										# )
df.facet_data_unpivot$clickjs <- paste0("alert(\"",df.facet_data_unpivot$tooltip, "\")" )

# df.facet_data_unpivot$tooltip <- sprintf('
# <div class="tipchart">
# <h4>%s</h4>
# <span class="tipspanstyle">Stats</span>
# <table>
# <tr class="tiprow">
# <td class="tipbarticks">Rate/000</td>
# <td class="tipbardiv"><div class="tipbar">%d</div></td>
# </tr>
# <tr class="tiprow">
# <td class="tipbarticks">Misses</td>
# <td class="tipbardiv"><div class="tipbar">%d</div></td>
# </tr>
# <tr class="tiprow">
# <td class="tipbarticks">Hits</td>
# <td class="tipbardiv"><div class="tipbar">%d</div></td>
# </tr>
# <tr class="tiprow">
# <td class="tipbarticks">Total</td>
# <td class="tipbardiv"><div class="tipbar">%d</div></td>
# </tr>
# </table>
# </div>',
...
#df.facet_data_unpivot$tooltip <- gsub("\\\n", "", df.facet_data_unpivot$tooltip)
...
ggplot_by_vehicle <- function(p_facet_category, p_facet_wrap_column_name, p_subset_column_name, p_snr_chart_category, p_snr_set_constraints, p_snr_miss_condition)
{
	...
	if (nrow(df.data_chart_category) == 0) {
		...
		#	geom_point_interactive(aes(size = bubble_size), alpha=0.4) +
		...
		htmlwidgets::saveWidget(widget=pp, file=snr_filename, selfcontained=TRUE)
